    ...
    if (!event)
      ...
      if (!event.registeredUsers.includes(currentUser._id)) {
        ...
        await sendEmail(currentUser.email, {
		  ...
		  <p>You have successfully registered to the event ${event.title.toUpperCase()}.</p>
		  <p>Event Date: ${event.dateTime.dates[0]}</p>

		  <p>Event Location: ${event.location.location}</p>
